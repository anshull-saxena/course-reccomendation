  ...
  return (
    <KeyboardAvoidingView behavior="padding" style={{ flex: 1 }}>
      <View style={styles.gradientBg}>
        ...
        {results && (
          <View style={styles.sectionCardFloating}>
            <Text style={styles.sectionTitle}><Text style={styles.sectionIcon}>🏆</Text> Results</Text>
            <Text style={styles.subHeading}>Recommended Courses for Next Semester</Text>
            {(!results.useCase1 || results.useCase1.length === 0 || (results.useCase1.length === 1 && !results.useCase1[0][0])) ? (
              <Text style={styles.emptyText}>No data available</Text>
            ) : results.useCase1.map((course, index) => (
              <Text key={index} style={styles.resultText}>{course[0]}</Text>
            ))}
            <Text style={styles.subHeading}>Predicted Final CGPA</Text>
            <Text style={styles.resultText}>Your Predicted CGPA by the end of 4-2 would be: {results.predictedCGPA}</Text>
            <Text style={styles.subHeading}>Learning Pathways to Improve Grade</Text>
            {(!results.learningPathways || results.learningPathways.length === 0 || (results.learningPathways.length === 1 && !results.learningPathways[0][0])) ? (
              <Text style={styles.emptyText}>No data available</Text>
            ) : results.learningPathways.map((course, index) => (
              <Text key={index} style={styles.resultText}>{course[0]}</Text>
            ))}
            <Text style={styles.subHeading}>Electives Personalization</Text>
            <Text style={styles.resultText}>Your preferred elective is: {preferredElective}</Text>
            {(!results.electivesPersonalization || results.electivesPersonalization.length === 0 || (results.electivesPersonalization.length === 1 && !results.electivesPersonalization[0][0])) ? (
              <Text style={styles.emptyText}>No data available</Text>
