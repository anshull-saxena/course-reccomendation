...
test_data = pd.read_csv('g.csv')
test_data = test_data[['subject', 'courseGrade']]


cdcs_file = pd.read_csv("C:\\Users\\11110\\OneDrive\\Desktop\\CDCs.csv")
...
def predict_gpa(similarities,this_student,i=25):
    cgpa_data = pd.read_csv('CGPA.csv').to_records(index=False)
    gpas = []
...
def recommend_courses(student_id, train_students, student_courses):
    ...
    csv_file_gpas = 'public/output_2.csv'
...
student_course = pd.read_csv("Grade_Student.csv",encoding='latin-1')
...
csv_file_path = 'public/output_3.csv'
...
csv_file_path = 'public/output_4.csv'
count_elements = 0
with open(csv_file_path, 'w', newline='') as csvfile:
    fieldnames = ['Course']
    csv_writer = csv.DictWriter(csvfile, fieldnames=['Course'])
